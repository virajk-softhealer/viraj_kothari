        ...
        if group_by:
            ...
            try:
                ...
                for group_dict in result_groups:
                    group_vals = [group_dict.get(g) for g in processed_groupby]
                    if operation == 'count':
                        agg_val = group_dict.get('__count', 0)
                    else:
                        agg_val = group_dict.get(field, 0)
                    ...
                    for g_val in group_vals:
                        if hasattr(g_val, 'display_name'):
                            ...
                            labels.append(str(g_val[1]) if len(g_val) > 1 else str(g_val[0]))
                        else:
            ...
            try:
                if operation == 'count':
                    ...
                    aggregates = [f'{field}:{operation}']
                    res = Model.read_group(domain=domain, fields=aggregates, groupby=[])
...
def _normalize_write_field_value(env, Model, field_name, raw_value, field_info):
    ...
    if field.type == "many2one":
        ...
        related_record.check_access_rights("read"); related_record.check_access_rule("read")
        return relation_id
    ...
    if field.type == "many2many":
        ...
        if relation_ids:
            ...
            related_records.check_access_rights("read"); related_records.check_access_rule("read")
...
def create_record(env, model=None, values=None, fields=None, records=None):
    ...
    try:
        if records:
            ...
            for item in records:
                ...
                try:
                    ...
                    try:
                        rec.check_access_rights("read"); rec.check_access_rule("read")
        ...
        if isinstance(values, list):
            ...
            try:
                records.check_access_rights("read"); records.check_access_rule("read")
                for rec in records:
...
def update_record(env, model=None, record_id=None, values=None, fields=None, records=None):
    ...
    try:
        if records:
            ...
            for item in records:
                ...
                try:
                    ...
                    rec.check_access_rights("write"); rec.check_access_rule("write")
        ...
        if isinstance(values, list):
            ...
            records.check_access_rights("write"); records.check_access_rule("write")
...
def delete_record(env, model=None, record_id=None, fields=None, records=None):
    ...
    try:
        if records:
            ...
            for item in records:
                ...
                try:
                    ...
                    rec.check_access_rights("unlink"); rec.check_access_rule("unlink")
        ...
        records.check_access_rights("unlink"); records.check_access_rule("unlink")
        records.unlink()
