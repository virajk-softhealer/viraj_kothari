...
class ShAiChatMessage(models.Model):
    ...
    def _set_processing_status(self, session_id, status):
        ...
        try:
            self.env['bus.bus']._sendone("sh_ai_assistance/processing_status", "sh_ai_assistance/processing_status", status_payload)
        except Exception:
    ...
    def _clear_processing_status(self, session_id):
        ...
        if session.exists():
            try:
                session.user_id._bus_send("sh_ai_assistance/processing_status", {
                    'active': False,
                    'session_id': session_id,
