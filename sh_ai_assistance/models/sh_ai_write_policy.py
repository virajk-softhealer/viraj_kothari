...
class ShAiWritePolicy(models.Model):
    """AI Write Restriction Rule for specific models."""
    ...
    def _check_model_id(self):
        """Ensure the selected model is not a protected technical or transient model."""
        for policy in self:
    ...
    def _onchange_block_update(self):
        """Automatically restrict archive access if updates are restricted."""
    ...
    def _onchange_block_archive(self):
        """Prevent allowing archive access if update access is restricted."""
    ...
    def create(self, vals_list):
        """Enforce internal policy consistency on creation."""
    ...
    def write(self, vals):
        """Update records and enforce internal policy consistency."""
        ...
        if "block_update" in vals or "block_archive" in vals:
            for record in self:
                if record.block_update and not record.block_archive:
                    ...
                    # SUDO: Required to bypass potential restrictions when enforcing internal policy consistency
                    record.sudo().block_archive = True
        return res


class ShAiWritePolicyField(models.Model):
    """Individual restricted fields within an AI Write Policy."""
    ...
    def _check_field_model(self):
        """Ensure the restricted field belongs to the policy's model."""
        for line in self:
