...
class ShAiWritePolicy(models.Model):
    ...
    _sql_constraints = [
        (
            "sh_ai_write_policy_model_company_uniq",
            "UNIQUE(model_id, company_id)",
            "Only one AI write rule is allowed per model and company.",
        ),
    ]
...
class ShAiWritePolicyField(models.Model):
    ...
    _sql_constraints = [
        (
            "sh_ai_write_policy_field_policy_field_uniq",
            "UNIQUE(policy_id, field_id)",
            "This field is already restricted on the selected AI write rule.",
        ),
    ]
