...
class ShAiChatSession(models.Model):
    ...
    def _compute_display_name(self):
        for session in self:
            session.display_name = session.name or _("Start New Conversation")
    ...
    def create(self, vals_list):
        # Safety net: if a single dict is passed instead of a list, wrap it
    ...
    def find_by_token(self, access_token):
        ...
        _logger.info(f"Finding session for token: {access_token} and user: {self.env.user.name} (ID: {self.env.user.id})")
        session = self.search([
            ('access_token', '=', access_token),
            ('user_id', '=', self.env.user.id)
        ], limit=1)
        _logger.info(f"Found session: {session.id if session else 'None'}")
