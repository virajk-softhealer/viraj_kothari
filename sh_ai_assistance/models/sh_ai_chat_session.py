...
class ShAiChatSession(models.Model):
    ...
    @api.depends('message_ids', 'message_ids.create_date', 'message_ids.prompt_tokens', 'message_ids.completion_tokens', 'message_ids.cached_tokens', 'message_ids.sh_reasoning_tokens', 'message_ids.tool_call_count', 'message_ids.execution_time', 'message_ids.total_tokens')
    def _compute_message_stats(self):
        for session in self:
            ...
            session.total_cached_tokens = sum(assistant_messages.mapped('cached_tokens'))
            session.total_reasoning_tokens = sum(assistant_messages.mapped('sh_reasoning_tokens'))
    ...
    def import_session_from_json(self, import_data, project_id=None):
        ...
        # 1. Handle Project Export (Multiple Sessions)
        if isinstance(import_data, dict) and import_data.get('export_type') == 'project':
            ...
            # Identify or create target project for this import batch
            ...
            for session_data in sessions_to_import:
                # Recursively call for each session using the identified project
        ...
        # 2. Handle Single Session (either legacy list or new dict)
        messages_data = []
        ...
        if not messages_data:
            # If there's a session record in the JSON but no messages, we still create it?
            # Usually users expect messages. If empty, return False.
            if not isinstance(import_data, dict) or 'messages' not in import_data:
                return False

        # Determine target project for this specific session
        target_project_id = project_id
