...
from ...sh_ai_base.provider.claude_provider import ClaudeProvider
        ...
        try:
            ...
            for iteration in range(self.max_iterations):
                ...
                if stop_event and stop_event.is_set():
                    _logger.info("🚫 Claude Engine: Stop signal received for session %s", llm_config.get("session_id"))
            ...
            if partial_message:
                self._mark_assistant_delta(context, partial_message.content or "")
            return self._build_result(
                context,
                clean_ai_response(partial_message.content if partial_message else ""),
                is_stopped=True,
            )

        except Exception as e:
            _logger.error("❌ Claude Engine Execution Error: %s", str(e), exc_info=True)
