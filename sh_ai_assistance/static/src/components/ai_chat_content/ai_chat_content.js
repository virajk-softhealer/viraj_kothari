...
import { Component, useState, useRef, onPatched, onWillUnmount, onWillUpdateProps } from "@odoo/owl";
...
export class AiChatContentComponent extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            cachedLlmCompany: this.props.selectedModel?.sh_company || this.props.currentLlmCompany || null,
            isListening: false,
            isSpeaking: false,
        ...
        if (this.busService.addChannel) {
            this.busService.addChannel("sh_ai_assistance/processing_status");
            this.busService.addEventListener("notification", ({ detail: notifications }) => {
                for (const notification of notifications) {
                    // const [type, status] = notification;
                    const { type, payload } = notification;
                    if (type === "sh_ai_assistance/processing_status") {
                        const status = payload;
                        if (!status || status.session_id !== this.props.sessionId) {
                            continue;
                        }
        ...
        onWillUpdateProps((nextProps) => {
            // Update cached company only if we get a valid one (stops flickering)
            const nextCompany = nextProps.selectedModel?.sh_company || nextProps.currentLlmCompany;
            if (nextCompany && nextCompany !== this.state.cachedLlmCompany) {
                this.state.cachedLlmCompany = nextCompany;
            }
    ...
    get currentUser() {
        return {
            id: session.uid || 1,
            name: session.name || session.partner_display_name || 'User',
        };
    }
    

    get providerWarningMessage() {
        // Use the cached company to ensure the UI is stable and doesn't flicker
        const company = this.state.cachedLlmCompany;
        if (!company) return null;

        const companyLower = company.toLowerCase();

        if (companyLower.includes('google') || companyLower.includes('gemini')) {
            return "Gemini models may make mistakes, so double-check outputs.";
        } else if (companyLower.includes('openai') || companyLower.includes('open ai') || companyLower.includes('gpt')) {
    ...
    get providerWarningTip() {
        return "Check the accuracy of the information provided by the AI before using it in official documents.";
    }
    ...
    async sendMessage() {
        ...
        try {
            ...
            // Call backend to process message using direct fetch to BYPASS Odoo's Global Blocker entirely
            try {
                const response = await fetch("/web/dataset/call_kw/sh.ai.chat.message/process_user_message", {
                    method: "POST",
                    headers: {
                    ...
                    body: JSON.stringify({
                        ...
                        method: "call",
                        params: {
                            model: "sh.ai.chat.message",
                            method: "process_user_message",
                            args: [this.props.sessionId, userMessage, messageId, this.state.viewPreference],
                            kwargs: {},
                        },
                ...
                const result = data.result || { error: data.error?.message || "Internal server error" };
