...
export class AiChatWindow extends Component {
    ...
    async createNewSession() {
        try {
            ...
            const idsList = Array.isArray(sessionIds) ? sessionIds : [sessionIds];
            const newSessions = await this.orm.read("sh.ai.chat.session", idsList, ['id', 'access_token', 'name']);
    ...
    async _getWebclientTarget() {
        const records = await this.orm.searchRead(
            "ir.model.data",
            [
                ["module", "=", "sh_ai_assistance"],
                ["name", "in", ["action_ai_assistance_dashboard", "menu_ai_chat_dashboard"]],
            ],
            ["name", "res_id"]
        );

        const xmlIds = records.reduce((accumulator, record) => {
            accumulator[record.name] = record.res_id;
            return accumulator;
        }, {});

        const hashParams = new URLSearchParams(window.location.hash.replace(/^#/, ""));
        const currentCids = hashParams.get("cids");
        const allowedCompanies = session.user_companies?.allowed_companies || {};
        const fallbackCompanyId = session.user_companies?.current_company || Object.keys(allowedCompanies)[0];

        return {
            actionId: xmlIds.action_ai_assistance_dashboard,
            menuId: xmlIds.menu_ai_chat_dashboard,
            cids: currentCids || (fallbackCompanyId ? String(fallbackCompanyId) : ""),
        };
    }

    async onExpand() {
        try {
            const target = await this._getWebclientTarget();
            if (!target.actionId || !target.menuId) {
                console.error("Failed to resolve AI Chat action or menu.");
                return;
            }

            const url = new URL(window.location.href);
            if (this.state.currentSessionToken) {
                url.searchParams.set("session", this.state.currentSessionToken);
            } else {
                url.searchParams.delete("session");
            }

            url.hash = `action=${target.actionId}&cids=${target.cids}&menu_id=${target.menuId}`;
            window.location.assign(url.toString());
        } catch (error) {
            console.error("Failed to open AI Chat dashboard:", error);
        }
    }
